...
Cand.mod <- list()
Cand.mod[["reg_only"]] <- lm(log(hgcA_gyrB_ratio, 10) ~ trans_reg,
                             data = hgcA_gyrB_ratio_data)
Cand.mod[["reg_sulfide_noInt"]] <- lm(log(hgcA_gyrB_ratio, 10) ~ trans_reg + sulfide_uM,
                                      data = hgcA_gyrB_ratio_data)
Cand.mod[["reg_sulfide_Int"]] <- lm(log(hgcA_gyrB_ratio, 10) ~ trans_reg * sulfide_uM,
                                    data = hgcA_gyrB_ratio_data)
Cand.mod[["reg_meta_noInt"]] <- lm(log(hgcA_gyrB_ratio, 10) ~ trans_reg + meta_code,
                                      data = hgcA_gyrB_ratio_data)
Cand.mod[["reg_meta_Int"]] <- lm(log(hgcA_gyrB_ratio, 10) ~ trans_reg * meta_code,
                                    data = hgcA_gyrB_ratio_data)
Cand.mod[["reg_class_noInt"]] <- lm(log(hgcA_gyrB_ratio, 10) ~ trans_reg + class,
                                   data = hgcA_gyrB_ratio_data)
Cand.mod[["reg_class_Int"]] <- lm(log(hgcA_gyrB_ratio, 10) ~ trans_reg * class,
                                 data = hgcA_gyrB_ratio_data)
Cand.mod[["sulfide"]] <- lm(log(hgcA_gyrB_ratio, 10) ~ sulfide_uM,
                            data = hgcA_gyrB_ratio_data)
Cand.mod[["metacode"]] <- lm(log(hgcA_gyrB_ratio, 10) ~ meta_code,
                             data = hgcA_gyrB_ratio_data)
Cand.mod[["class"]] <- lm(log(hgcA_gyrB_ratio, 10) ~ class,
                             data = hgcA_gyrB_ratio_data)
