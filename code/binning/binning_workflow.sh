...
do
  if [ ! -e original_summaries/uncurated_bins_from_$assembly ]; then
    ...
    echo $assembly "already summarized"
          ...
          awk -F ',' '{ print $2 }' ~/BLiMMP/metadata/lists/assembly_key.csv | \
            sort | uniq | \
            while read year
          do
            initialAssembly=`awk -F ',' -v year="$year" '$2 == year { print $1 }' ~/BLiMMP/metadata/lists/assembly_key.csv | head -n 1`
            head -n 1 $initialAssembly\_coverage.txt > coverage_$year.txt

            awk -F ',' -v year="$year" '$2 == year { print $1 }' ~/BLiMMP/metadata/lists/assembly_key.csv | while read assembly
            do
          ...
          conda activate bioinformatics
          PYTHONPATH=""
          scripts=/home/GLBRCORG/bpeterson26/BLiMMP/code/generalUse
          binsGood=~/BLiMMP/dataEdited/binning/manualBinning/binsGood
          ...
          # Run METABOLIC
          #########################
          cd ~/BLiMMP/dataEdited/binning/manualBinning/binsGood/metabolism/forMETABOLIC
          condor_submit ~/BLiMMP/code/submission/METABOLIC_processing.sub
          ...
          PERL5LIB=""
          binsGood=~/BLiMMP/dataEdited/binning/manualBinning/binsGood
          KOFAM_output=/home/GLBRCORG/bpeterson26/BLiMMP/dataEdited/binning/manualBinning/binsGood/metabolism/KOFAM_output
          ORFs=/home/GLBRCORG/bpeterson26/BLiMMP/dataEdited/binning/manualBinning/binsGood/ORFs
          ...
          cd ~/BLiMMP/dataEdited/binning/manualBinning/binsGood/metabolism
          mkdir FeGenie
          cp /home/GLBRCORG/cnolmsted/FeGenie_EET_Stuff/PipelineOutput/All_EET_Proteins_bothMethods_BLiMMP13genomes.csv \
              FeGenie
          ...
          #########################
          # On local computer
          cd /Users/benjaminpeterson/Documents/research/BLiMMP/dataEdited/binning/bin_orfs
