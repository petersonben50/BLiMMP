...
plot.Hg.species.over.incubation <- function(date_to_use,
  ...
  plot.to.show <- plotting.data %>%
    ggplot(aes(x = durationInDays,
               ...
               color = treatment)) +
    geom_point(aes(shape = MeHg_198_above_DDL)) +
...
concentration.to.use <- c(-0.01, 0.48)
constituent.ylab <- "MeHg (ng/L)"
...
pdf("results/figures/incubations_Me198Hg_conc.pdf",
    width = 7,
    heigh = 7)
...
#### Check for detectable MeHg in filtered controls ####
filter_controls <- Hg_inc_data %>%
  filter(treatment == "filtered-unamended")

#### Check for detectable MeHg in t0 samples ####
t0_samples <- Hg_inc_data %>%
  filter(t == "t0")
