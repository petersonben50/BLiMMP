...
library(tidytree)
taxonomic_grouping_hgcA <- list()
metabolic_grouping_hgcA <- list()

taxonomic_grouping_hgcA[["Firmicutes"]] <- c("BLI21_coassembly_000000219078_2", "BLI21_coassembly_000000003920_15", "BLI21_coassembly_000000858312_1")
taxonomic_grouping_hgcA[["Spirochaetes"]] <- c("BLI21_coassembly_000000277987_1", "BLI20_coassembly_000000214677_1", "BLI21_coassembly_000000397409_2")
taxonomic_grouping_hgcA[["Actinobacteria"]] <- c("BLI21_assembly104_000000083274_2", "BLI21_coassembly_000000136593_2", "BLI21_coassembly_000000018594_7",
                                               "BLI21_assembly105_000000164796_1")
taxonomic_grouping_hgcA[["Planctomycetes"]] <- c("BLI21_coassembly_000000724865_2",
...
taxonomic_grouping_hgcA[["Kiritimatiellaeota"]] <- c("BLI21_coassembly_000000061249_5",
...
Lenti.seqs <- tree_subset(hgcA.tree,
                          ...
                          levels_back = 0)$tip.label
taxonomic_grouping_hgcA[["Lentisphaerae"]] <- c(grep("BLI2[0:1]_", Lenti.seqs, value = TRUE),
...
unknown.PVC.seqs <- tree_subset(hgcA.tree,
                          ...
                          levels_back = 0)$tip.label
taxonomic_grouping_hgcA[["Unknown_PVC"]] <- c(grep("BLI2[0:1]_", unknown.PVC.seqs, value = TRUE))
rm(unknown.PVC.seqs)

taxonomic_grouping_hgcA[["Bacteroidetes"]] <- c("BLI21_assembly102_000000112326_2")
taxonomic_grouping_hgcA[["Geobacterales"]] <- c("BLI21_coassembly_000000229519_3")
taxonomic_grouping_hgcA[["Unknown"]] <- c("BLI21_coassembly_000001026396_2",
                                          "BLI21_coassembly_000000438757_1")
taxonomic_grouping_hgcA[["Desulfobacterales"]] <- c("BLI20_assembly002_000000161823_1", "BLI21_assembly106_000000022489_2", "BLI21_coassembly_000000058806_3", "BLI21_coassembly_000000004570_14",
...
unknown.distant.seqs <- tree_subset(hgcA.tree,
                          ...
                          levels_back = 0)$tip.label
taxonomic_grouping_hgcA[["Unknown_distant"]] <- c(grep("BLI2[0:1]_", unknown.distant.seqs, value = TRUE))
rm(unknown.distant.seqs)

grep("BLI2[0:1]_", hgcA.tree$tip.label, value = TRUE)[which(!(grep("BLI2[0:1]_", hgcA.tree$tip.label, value = TRUE) %in% unlist(taxonomic_grouping_hgcA)))]

taxonomic_grouping_hgcA.df <- data.frame()
for (taxa in names(taxonomic_grouping_hgcA)) {
  temp.df <- data.frame(seqID = taxonomic_grouping_hgcA[[taxa]],
                        taxa_by_tree = rep(taxa, length(taxonomic_grouping_hgcA[[taxa]])))
  taxonomic_grouping_hgcA.df <- rbind(taxonomic_grouping_hgcA.df,
                                      temp.df)
  rm(temp.df)
}
```
...
hgcA.data <- left_join(hgcA.data,
                       left_join(taxonomic_grouping_hgcA.df,
                                 hgcA.data %>%
                                   select(seqID, cluster_ID)) %>%
...
# Assign metabolic potential

```{r assignment_metabolic_potential }
metabolic_guilds <- c("FERM", "FERM", "SRB", "FERM", "HRRO", "FERM", "FERM", "UNK", "UNK", 'UNK', 'UNK', 'FERM')
names(metabolic_guilds) <- sort(unique(hgcA.data$taxa_by_tree))
hgcA.data <- hgcA.data %>%
  mutate(metabolic_guild = metabolic_guilds[taxa_by_tree])
```
