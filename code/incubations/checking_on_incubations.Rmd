...
Let's check out the Hg data first. 

``` {r incubations BLiMMP_trip_003, echo=FALSE, message = FALSE}
Hg.incubation.data <- read.csv("/Users/benjaminpeterson/Box/BLiMMP/dataEdited/incubations/MeHg/incubations2019_Me198Hg.csv",
                            stringsAsFactors = FALSE) 

# Pull out needed stat
plotting.data <- Hg.incubation.data %>%
  filter(tripID == "BLiMMP_trip_003") %>%
  select(incubationID, depth, startDate, t, treatment, excess_MeHg_198_ng.L) %>%
  spread(key = t,
...
par(mfrow = c(1, 2),
    mar = c(4.5, 4.5, 3, 1))
invisible(
sapply(sort(unique(plotting.data$depth)),
       function(depth.of.interest) {
         {
           
             y_max <- ceiling(max(c(plotting.data$t0, plotting.data$t1))*100)/100
             y_min <- floor(min(c(plotting.data$t0, plotting.data$t1))*100)/100

           
           plotting.data.depth <- plotting.data %>%
             filter(depth == depth.of.interest)
           
            stripchart(list(plotting.data.depth$t0,
                            plotting.data.depth$t1),
              vertical = TRUE,
              cex = 0,
              ylim = c(y_min, y_max),
              group.names = c("t=0", "t=24"),
              main = paste("Me198Hg production\nat ",
                           depth.of.interest,
                           "m",
                           sep = ""))
            
           for (treatment.of.interest in unique(plotting.data.depth$treatment)) {
                    
                    plotting.data.treatment <- plotting.data.depth %>%
                      filter(treatment == treatment.of.interest)
                    
                    stripchart(list(plotting.data.treatment$t0,
                                    plotting.data.treatment$t1),
                      vertical = TRUE,
                      pch = 18,
                      group.names = c("t=0", "t=24"),
                      col = color.vector[plotting.data.treatment$treatment],
                      add = TRUE)
           }
            
            for (row.num in 1:nrow(plotting.data.depth)) {
              segments(1, plotting.data.depth$t0[row.num],
                       2, plotting.data.depth$t1[row.num],
                       col = color.vector[plotting.data.depth$treatment[row.num]])
            }
            text(y = plotting.data.depth$t1,
                 x= 1.75, 
                 labels = plotting.data.depth$incubationID,
                 cex = 0.8)
...
Additionally (and interestingly), I noticed that the sulfide levels in this bag at the end of the incubation were substantially lower than in the other bags from this depth.
I doubt this is related to withdrawing the spike early, more likely just some O2 intrusion into the bag, which could also account for the lower Me198Hg production. 

```{r sulfide_data_incubations_BLiMMP_trip_003,  echo=FALSE, message = FALSE, fig.height = 5, fig.width = 5}

sulfide.data <- read.csv("/Users/benjaminpeterson/Box/BLiMMP/dataEdited/waterChemistry/sulfide/MA_data.csv",
                         stringsAsFactors = FALSE) %>%
  filter(tripID == "BLiMMP_trip_003")
...
Sulfide was quite low at this point, only 5.38µM.
...
Leucine uptake rates were fastest at the sample from the oxic-anoxic interface, approximately twice as high as they were at the other two depths.
...
graph.path <- "/Users/benjaminpeterson/Box/BLiMMP/results/incubations/BLiMMP_trip_005_incubations.png"
...
sapply(c(14, 20.1),
       function(depth.of.interest) {
         ...
         sulfide.data <- read.csv("/Users/benjaminpeterson/Box/BLiMMP/dataEdited/waterChemistry/sulfide/MA_data.csv",
            ...
            left_join(read.csv("/Users/benjaminpeterson/Box/BLiMMP/dataEdited/incubations/MeHg/incubations2019_Me198Hg.csv",
                               stringsAsFactors = FALSE) %>%
                        filter(t == "t1"))
          
          WC.S.levels <- read.csv("/Users/benjaminpeterson/Box/BLiMMP/dataEdited/waterChemistry/sulfide/WC_data.csv",
                                   stringsAsFactors = FALSE) %>%
            filter(tripID == trip.of.interest,
                   depth == depth.of.interest) %>%
            select(S_conc_uM) %>%
...
# Read in a PDF of the document
graph.path <- "/Users/benjaminpeterson/Box/BLiMMP/results/incubations/BLiMMP_trip_006_incubations.png"
include_graphics(graph.path)
```
...
original.uptake <- read.csv("/Users/benjaminpeterson/Box/BLiMMP/dataEdited/leucineUptake/water_column_uptake.csv",
                            stringsAsFactors = FALSE) %>%
...
leucine.uptake.MA <- read.csv("/Users/benjaminpeterson/Box/BLiMMP/dataEdited/leucineUptake/incubation_uptake.csv",
                              stringsAsFactors = FALSE) %>%
