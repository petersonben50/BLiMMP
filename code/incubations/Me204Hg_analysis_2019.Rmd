...
The samples from 2019-07-30 are odd, with MeHg fractions reaching up int 115-140%, which should not be the case.
...
It is likely the former, since we see a consistent decrease across all conditions (even the filtered bags), and the preincubations are just filtered sample water.
If it can happen in the filtered control incubation, it can happen in the pre-equilibration step.

We see this again on 2019-10-08.
At 16.2m , the start values are pretty variable, ranging from ~70% to just under 100%.
However, once the incubation starts, the values drop slightly.
...
Wonder if this is due to elimination of biotic MeHg production, so that replenishment of MeHg by methylation in the unfiltered samples means there's less of a demethylation signature?
But, molybdate knocks out methylation too... so that can't be it...

Finally, the October samples, BLiMMP_006
...
This drop holds up in the filtered samples, so again, support for it being an abiotic mechanism.
...
Molybdate-amendment doesn't appear to have a clear impact on MeHg degradation.
